...
app.get("/profile", (req, res) => {
   ...
   if (token) {
      jwt.verify(token, jwtSecret, {}, async (err, userData) => {
         if (err) throw err;
         const { name, email, _id,hobbies } = await UserModel.findById(userData.id);
         res.json({ name, email, _id,hobbies });
      });
   } else {
      res.json(null);
   }
...
app.put("/profile/:userId", async (req, res) => {
   ...
   try {
     const updatedUser = await UserModel.findByIdAndUpdate(userId, { hobbies }, { new: true });
     res.json(updatedUser);
   } catch (error) {
     console.error('Error updating user hobbies:', error);
     res.status(500).json({ error: 'Failed to update user hobbies' });
   }
 });
